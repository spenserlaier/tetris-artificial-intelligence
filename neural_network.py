...
class DQNAgent:
    ...
    def choose_action(self, state, exploration_prob):
        if numpy.random.rand() < exploration_prob:
            ...
            q_values = self.model.predict(numpy.reshape(state, (1, self.state_size)),verbose=0)
            #print("q_values: ")
    ...
    def update_q_values(self, state, action, reward, next_state, done):
        target = reward
        if not done:
            target += 0.9 * numpy.max(self.model.predict(numpy.reshape(next_state, (1, self.state_size)), verbose=0))

        target_f = self.model.predict(numpy.reshape(state, (1, self.action_size)),verbose=0)
        target_f[0][action] = target
        self.model.fit(numpy.reshape(state, (1, self.state_size)), target_f, epochs=1, verbose=0) #can set verbose to 0 later
